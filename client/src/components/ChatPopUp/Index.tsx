...
import style from './ChatPopUp.module.scss';
...
import ClientImageWithFallback from '../ImageWithFallback';
...
const ChatPopUp = ({ show, ChatRoom, handleClose }: ChatPopUpProps) => {
  ...
  const fetchData = async () => {
    ...
    console.log(sessionUse?.user.image);
  };
  ...
  return (
    <>
      {show && (
        <MDBContainer className={cx('chatBoxContainer', 'py-5')}>
          <MDBRow className='d-flex justify-content-end me-2'>
            <MDBCol
              md='8'
              lg='6'
              xl='4'
            >
              <MDBCard>
                ...
                <Row
                  style={{ overflowY: 'scroll', position: 'relative', height: '400px' }}
                >
                  <MDBCardBody>
                    {mesageList.length != 0 &&
                      mesageList.map((m, i) => {
